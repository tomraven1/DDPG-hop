...
function [nstep,y_tot,t_event,xt_tot,y_event,epsilon,conv]=USLIP_dim(Pin,Cin,IC,epsilon,ang,elong)
...
m=Pin(1); M=Pin(2); k=Pin(3); rhow=Pin(4); g=Pin(5); V=Pin(6); drmax=Pin(8); Ax=Pin(9); Ay=Pin(10); cdx=Pin(11); cdy=Pin(12);
%Cin=[rs,alfa];
rs=double(elong); 
 r0=Pin(7);
%alfa=Cin(2);
...
maxstep=length(epsilon);  
    ...
    for nstep=1:maxstep
        ...
        % xoverFcn = @(T, Y,err) eventsTDdisturbed(T, Y,err, alfa);
...
%global r0 alfa
 ...
 global r0 alfa %net dens vel test_check 
% 
% %%% CONTROLLER 
% if y(4)<0.5&&y(4)>-0.5 && t>0.01 %% condition of peak - this can be improved
